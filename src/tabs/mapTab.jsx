...
export default function Map() {
    ...
    const statusChange = async (id) => {
        try {
            // Update the status of the report to 'collected' in Firestore
            const docRef = firebase.firestore().collection("generalUsersReports").doc(id);
            await docRef.update({
                status: 'collected',
            });
    
            // Update the filter state to 'collected' to display collected reports on the map
            setFilter('collected');
        } catch(e) {
            console.error(e);
        }
        ...
        async function fetchUsernameForSelectedMarker() {
            try {
                if (selectedMarker && selectedMarker.userId) {
                    ...
                    setUsername(fetchedUsername); // Update the state variable with the fetched username
                }
            } catch (error) {
                console.error('Error fetching username:', error);
            }
                                    ...
                                    <div style={{ width: '300px', display: 'flex', flexDirection: 'row' }}>
                                            ...
                                            <button 
                                                style={{ 
                                                    marginTop: 13, 
                                                    borderRadius: 10, 
                                                    backgroundColor: filter === 'collected' ? 'red' : 'green', 
                                                    borderColor: 'white', 
                                                    color: 'white', 
                                                    fontWeight: 'bold',
                                                    display: filter === 'uncollected' ? 'block' : 'none' // Set display property based on filter
                                                }}
                                                onClick={() => {
                                                    handleFilterButtonClick('collected'); // Update filter to 'collected'
                                                    statusChange(selectedMarker.id); // Call statusChange function with report ID
                                                }}
                                            >
                                                {filter === 'collected' ? 'Collected' : 'Collect'}
                                            </button>
                                        )}
