...
async function fetchReportsByStatusAndMunicipality(status, userMunicipality) {
    const q = query(collection(db, 'generalUsersReports'), where('status', '==', status), where('municipality', '==', userMunicipality));
...
export default function Map() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const reportsData = await fetchReportsByStatusAndMunicipality('uncollected', userMunicipality);
    ...
    const handleFilterButtonClick = useCallback(async (status) => {
        setFilter(status);
        try {
            const reportsData = await fetchReportsByStatusAndMunicipality(status, userMunicipality);
            setReports(reportsData);
        } catch (error) {
